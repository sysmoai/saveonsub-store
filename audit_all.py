...
for pg in sorted(pages):
    ...
    # RICH-RESULTS fields. These products are digital subscription/access
    # offers. Do not invent rolling price expiry dates or a physical-goods
    # ReturnByMail policy merely to satisfy the audit. Those properties are
    # optional; if present, they must be structurally valid.
                ...
                if isinstance(of, dict) and of.get('@type') == 'AggregateOffer':
                    ...
                    for sub in of.get('offers', []):
                        for k in ('price','priceCurrency','availability','url'):
                            if k not in sub: issues.append(f"{pg}: Offer no {k}")
                        if 'priceValidUntil' in sub:
                            pv = str(sub.get('priceValidUntil',''))
                            if not re.fullmatch(r'\d{4}-\d{2}-\d{2}', pv):
                                issues.append(f"{pg}: Offer invalid priceValidUntil")
                        if 'hasMerchantReturnPolicy' in sub:
                            rp = sub.get('hasMerchantReturnPolicy')
                            if not isinstance(rp, dict) or rp.get('@type') != 'MerchantReturnPolicy':
                                issues.append(f"{pg}: Offer invalid return policy")
            if ty == 'FAQPage':
    ...
    for l in set(re.findall(r'href="([^"#?]+)"', h)):
        if l.startswith(('http','mailto','tel',"'")) or any(c in l for c in ('${','+','`',"'")): continue
...
if issues:
    print(f"❌ {len(issues)} GAPS:")
    for x in issues[:25]: print(' -', x)
    sys.exit(1)
print("✅ ZERO GAPS — full regression pass")
