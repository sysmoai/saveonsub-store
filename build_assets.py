...
# The cache name MUST change whenever a cached asset changes. It was previously
# the frozen literal 'sos-v1', so activate() never purged anything and returning
# visitors were served assets/catalog.js — the price database — from a cache-first
# bucket that never expired. A price correction could not reach them at all.
# Deriving it from asset content makes every meaningful build self-invalidating.
import hashlib as _hashlib
_cache_inputs = ['assets/style.css', 'assets/app.js', 'assets/catalog.js']
_h = _hashlib.sha256()
for _f in _cache_inputs:
    try:
        _h.update(open(_f, 'rb').read())
    except OSError:
        pass  # asset not built yet on a partial run; hash still changes downstream
_CACHE_NAME = 'sos-' + _h.hexdigest()[:12]
...
'''.replace('__CACHE_NAME__', _CACHE_NAME)
open('sw.js', 'w').write(SW)
print(f'OK: sw.js cache={_CACHE_NAME}')
